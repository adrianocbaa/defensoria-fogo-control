...
CREATE OR REPLACE FUNCTION public.confirm_maintenance_service(p_token uuid, p_accept boolean DEFAULT true, p_note text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
  ...
  IF p_accept THEN
    UPDATE public.maintenance_tickets
       SET confirmed_at = now(),
           confirmed_source = 'requester',
           finalized_at = now(),
           status = 'Concluído'
     WHERE id = v_ticket.id;

    -- Enfileira geração do PDF de arquivamento
    INSERT INTO public.maintenance_ticket_email_outbox (ticket_id, kind, payload, scheduled_for, status)
    VALUES (v_ticket.id, 'auto_finalize', '{}'::jsonb, now(), 'pending');
  ELSE
    UPDATE public.maintenance_tickets
