...
BEGIN
  ...
  orcamento AS (
    SELECT h.id, h.item, h.quantidade, h.origem,
      CASE
        -- Itens extracontratuais: quantidade já é o valor do aditivo, não aplica ajuste duplicado
        WHEN h.origem = 'extracontratual' THEN GREATEST(0, h.quantidade)
        -- Itens contratuais: aplica ajuste de aditivos (supressões/acréscimos)
        ELSE GREATEST(0, h.quantidade + COALESCE(a.ajuste, 0))
...
BEGIN
  ...
  orcamento AS (
    SELECT h.obra_id, h.id, h.item, h.quantidade, h.origem,
      CASE
        WHEN h.origem = 'extracontratual' THEN GREATEST(0, h.quantidade)
        ELSE GREATEST(0, h.quantidade + COALESCE(a.ajuste, 0))
      END as quantidade_ajustada
