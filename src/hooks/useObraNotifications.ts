 ...
 * Baseado nos logs de ações (obra_action_logs) e logs de RDO (rdo_audit_log)
 */
export function useObraNotifications() {
  ...
  const { data: notifications, isLoading, refetch } = useQuery({
    queryKey: ['obra-notifications', user?.id],
    queryFn: async (): Promise<ObraNotification[]> => {
      ...
      (rdoLogs || []).forEach(log => {
        const actionMap: Record<string, string> = {
          ...
          'APROVAR': 'RDO aprovado',
          'REABRIR': 'RDO reaberto',
        };
