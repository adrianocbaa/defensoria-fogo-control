...
const createCustomIcon = (color: string) => {
  ...
  ">
    <div style="
      ...
      box-shadow: 0 2px 8px rgba(0,0,0,0.2);
      position: absolute;
      animation: pulse-${color.replace('#', '')} ${animationSpeed} infinite;
    "></div>
    <div style="
      width: 20px;
      height: 20px;
      ...
      position: absolute;
      animation: pulse-ring-${color.replace('#', '')} ${animationSpeed} infinite;
    "></div>
  </div>
  <style>
    ...
    @keyframes pulse-ring-${color.replace('#', '')} {
...
export function MapView({ nuclei, onViewDetails }: MapViewProps) {
  ...
  const NucleusDetailsContent = ({ nucleus }: { nucleus: Nucleus }) => (
    <div className="space-y-3">
      {/* Address */}
      <div>
        <p className="text-sm font-medium mb-1">Endereço:</p>
        <p className="text-sm text-muted-foreground">{nucleus.address}</p>
      </div>
      ...
      {nucleus.isAgentMode === true ? (
        ...
        <>
          ...
          <div className="flex items-center gap-2">
            <Droplets className={`h-4 w-4 ${nucleus.hydrants.length > 0 ? 'text-blue-600' : 'text-gray-400'}`} />
          ...
          {nucleus.fireExtinguishers.some(ext => ext.status === 'expired') && (
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-danger">
                {nucleus.fireExtinguishers.filter(ext => ext.status === 'expired').length} extintor(es) vencido(s)
              </span>
            </div>
          )}
        </>
      ...
      {nucleus.fireDepartmentLicense?.validUntil && (
        <div className="flex items-center gap-2">
          {(() => {
            const now = new Date();
            const validUntil = new Date(nucleus.fireDepartmentLicense.validUntil);
        ...
        <div className="space-y-2 pt-2 border-t">
          {nucleus.contact.phone && (
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs">{nucleus.contact.phone}</span>
            </div>
          )}
          {nucleus.contact.email && (
            <div className="flex items-center gap-2">
              <Mail className="h-3 w-3 text-muted-foreground" />
              <span className="text-xs">{nucleus.contact.email}</span>
            </div>
          )}
        </div>
      )}
  ...
  return (
    <div className="relative w-full h-[600px] border rounded-lg overflow-hidden z-0">
      ...
      {/* Desktop: Nucleus info panel */}
      {selectedNucleus && !isMobile && (
      ...
      {selectedNucleus && isMobile && showMobileModal && (
        <div className="fixed inset-0 bg-black/50 z-[1001] flex items-end justify-center p-4">
          <div className="bg-white rounded-t-lg w-full max-w-md max-h-[80vh] overflow-y-auto animate-slide-in-right">
      ...
      {/* Mobile: Compact nucleus list at bottom */}
      {isMobile && nuclei.length > 0 && (
        <div className="absolute bottom-4 left-4 right-4 bg-white rounded-lg shadow-lg border z-[1000]">
          <div className="p-3">
            ...
            <div className="flex gap-2 overflow-x-auto pb-2">
              {nuclei.map((nucleus) => (
                <button
                  ...
                  className={`flex-shrink-0 p-2 rounded border text-left min-w-[120px] ${
                    selectedNucleus?.id === nucleus.id ? 'bg-primary/10 border-primary' : 'bg-card hover:bg-muted'
                  }`}
                >
                  <div className="font-medium text-xs truncate">{nucleus.name}</div>
                  <div className="text-muted-foreground text-xs truncate">{nucleus.city}</div>
                  <div className="flex items-center gap-1 mt-1">
                    {nucleus.isAgentMode === true ? (
                      ...
                      <>
                        ...
                        {!nucleus.fireExtinguishers.some(ext => ext.status === 'expired' || ext.status === 'expiring-soon') && (
